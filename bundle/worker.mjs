  ...
  async doctor() {
    ...
    if (version.code !== 0) {
      return {
        ...
        message: version.stderr || "Codex CLI nicht verf\xFCgbar"
      };
    }
...
var RepositoryScope = external_exports.object({
  ...
  const root = resolve2(allowedRoot);
...
function bootstrapAuth(codexHomeValue, credentialBase64) {
  if (!/^[A-Za-z0-9+/]*={0,2}$/.test(credentialBase64)) throw new Error("Credential-Payload ist kein g\xFCltiges Base64");
...
async function loginAccessToken(codexBin, codexHomeValue, tokenBase64) {
  if (!/^[A-Za-z0-9+/]*={0,2}$/.test(tokenBase64)) throw new Error("Token-Payload ist kein g\xFCltiges Base64");
  const token = Buffer.from(tokenBase64, "base64");
  if (token.length === 0 || token.length > 64 * 1024) {
    throw new Error("Token-Payload muss zwischen 1 Byte und 64 KiB gro\xDF sein");
  }
...
async function executeWorkerRequest(input, onEvent = () => {
}) {
...
for await (const chunk of process.stdin) {
  ...
  size += buffer.length;
  if (size > 22e5) throw new Error("Worker-Request \xFCberschreitet 2,2 MiB");
  chunks.push(buffer);
}
